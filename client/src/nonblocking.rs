...
pub trait Signer: solana_sdk::signature::Signer + Send + Sync {}

impl<T: solana_sdk::signature::Signer + Send + Sync> Signer for T {}

/// Collection of signers.
///
/// We need a wrapper type because we cannot implement `Signers` trait on
/// `Vec<&den Signer>`.
#[derive(Clone, Default, bytemuck::TransparentWrapper)]
#[repr(transparent)]
